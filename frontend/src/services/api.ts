...
export const profileApi = {
  ...
  update: (data: {
    ...
    api.put<{ success: boolean; message: string; data: AuthUser }>(
      "/profile",
      data,
    ),
...
/**
 * Interface data Unit kerja rumah sakit.
 */
...
export const attendanceApi = {
  ...
  checkOut: (
    ...
    if (keteranganLembur)
      formData.append("keterangan_lembur", keteranganLembur);
    return api.post<{
      success: boolean;
      message: string;
      data: AttendanceRecord;
  ...
  earlyCheckouts: (status?: string, month?: number, year?: number) => {
    ...
    return api.get<{ success: boolean; data: AttendanceRecord[] }>(
      `/attendance/early-checkouts${query}`,
    );
  },
  // Setujui pulang cepat
  approveEarlyCheckout: (id: number, adminNote?: string) =>
    api.put<{ success: boolean; data: AttendanceRecord }>(
      `/attendance/${id}/early-checkout/approve`,
      { admin_note: adminNote },
    ),
  // Tolak pulang cepat
  rejectEarlyCheckout: (id: number, adminNote: string) =>
    api.put<{ success: boolean; data: AttendanceRecord }>(
      `/attendance/${id}/early-checkout/reject`,
      { admin_note: adminNote },
    ),
  // Memperbarui catatan lembur setelah check-out berhasil
  updateOvertimeNote: (overtimeNote: string) =>
    api.put<{ success: boolean; message: string; data: AttendanceRecord }>(
      "/attendance/overtime-note",
  ...
  approveOvertime: (id: number, adminNote?: string) =>
    api.put<{ success: boolean; data: AttendanceRecord }>(
      ...
      { overtime_admin_note: adminNote },
    ),
  // Tolak lembur
  rejectOvertime: (id: number, adminNote: string) =>
    api.put<{ success: boolean; data: AttendanceRecord }>(
      `/attendance/${id}/overtime/reject`,
      { overtime_admin_note: adminNote },
    ),
...
export const leaveApi = {
  ...
  list: (params?: { personal?: string }) =>
    api.get<{ success: boolean; data: LeaveRequest[] }>(
      "/leave-requests" +
        (params?.personal ? `?personal=${params.personal}` : ""),
    ),
  ...
  create: (
    ...
    api.post<{ success: boolean; message: string; data: LeaveRequest }>(
      "/leave-requests",
      data,
    ),
  ...
  delete: (id: number) =>
    api.delete<{ success: boolean }>(`/leave-requests/${id}`),
  // Menghapus semua pengajuan cuti yang sudah ditinjau (approved/rejected)
  deleteAllProcessed: () =>
    api.delete<{ success: boolean }>("/leave-requests/all-processed"),
  ...
  shortenAdmin: (
    ...
    shortened_reason: string,
  ) =>
    api.put<{ success: boolean; data: LeaveRequest }>(
      `/leave-requests/${id}/shorten`,
      { actual_end_date, shortened_reason },
    ),
  // Admin mendeteksi kemungkinan pegawai kembali lebih awal
  possibleEarlyReturns: () =>
    api.get<{
      success: boolean;
      data: Array<{ leave_request: LeaveRequest; detected_dates: string[] }>;
    }>("/leave-requests/possible-early-returns"),
};
...
export const specialLeaveApi = {
  listActive: () =>
    api.get<{ success: boolean; data: SpecialLeaveCategory[] }>(
      "/special-leave-categories",
    ),
  create: (name: string) =>
    api.post<{ success: boolean; message: string; data: SpecialLeaveCategory }>(
      "/special-leave-categories",
      { name },
    ),
...
export const notificationApi = {
  ...
  deleteAllRead: () =>
    api.delete<{ success: boolean }>("/notifications/delete-read"),
};
...
export const reportApi = {
  ...
  summary: (month?: number, year?: number) => {
    ...
    return api.get<{ success: boolean; data: ReportSummary }>(
      `/reports/summary${qs}`,
    );
  },
...
export interface EmployeeMonthlySchedule {
  ...
  department?: number;
  role?: string;
  pj_department_name?: string | null;
  dates: Record<
    string,
    {
      schedule_id: number;
...
export const scheduleApi = {
  ...
  update: (
    id: number,
    data: Partial<Omit<ShiftSchedule, "id" | "employees_count">>,
  ) =>
    api.put<{ success: boolean; data: ShiftSchedule }>(
      `/schedules/${id}`,
      data,
    ),
  // Hapus shift kerja
  ...
  getEmployeeSchedules: () =>
    api.get<{ success: boolean; data: EmployeeWeeklySchedule[] }>(
      "/employee-schedules",
    ),
  ...
  assignEmployeeSchedule: (
    ...
    api.post<{ success: boolean; message: string }>(
      ...
      { employee_id, day_of_week, schedule_id },
    ),
  ...
  mySchedule: () =>
    api.get<{
      success: boolean;
      data: MyShiftSchedule | null;
      saturday_shift?: MyShiftSchedule | null;
      day: string;
      source?: "date_specific" | "weekly";
    }>("/my-schedule"),
...
export const holidayApi = {
  ...
  create: (data: { date: string; name: string }) =>
    api.post<{ success: boolean; message: string; data: Holiday }>(
      "/holidays",
      data,
    ),
  ...
  update: (id: number, data: { date: string; name: string }) =>
    api.put<{ success: boolean; message: string; data: Holiday }>(
      `/holidays/${id}`,
      data,
    ),
  ...
  listAssignments: (holidayId: number) =>
    api.get<{ success: boolean; data: HolidayWorkAssignment[] }>(
      `/holidays/${holidayId}/work-assignments`,
    ),
  ...
  unassign: (holidayId: number, employeeId: number) =>
    api.delete<{ success: boolean; message: string }>(
      `/holidays/${holidayId}/work-assignments/${employeeId}`,
    ),
};
...
export const adminManagementApi = {
  list: () =>
    api.get<{ success: boolean; data: AuthUser[] }>("/super-admin/admins"),
  create: (data: {
    name: string;
    username: string;
    email: string;
    ...
    api.post<{ success: boolean; message: string; data: AuthUser }>(
      "/super-admin/admins",
      data,
    ),
  update: (
...
export const shiftProposalApi = {
  list: (params?: { status?: string; department_id?: string }) => {
    ...
    if (params?.department_id)
      query.append("department_id", params.department_id);
    return api.get<{ success: boolean; data: ShiftProposal[] }>(
      `/shift-assignment-proposals?${query.toString()}`,
    );
  },
  ...
  create: (data: {
    ...
    return api.post<{ success: boolean; message: string; data: ShiftProposal }>(
      "/shift-assignment-proposals",
      data,
    );
  },

  approve: (id: number, adminNote?: string) => {
    return api.put<{ success: boolean; message: string; data: ShiftProposal }>(
      `/shift-assignment-proposals/${id}/approve`,
      { admin_note: adminNote },
    );
  },
  ...
  reject: (id: number, adminNote: string) => {
    return api.put<{ success: boolean; message: string; data: ShiftProposal }>(
      `/shift-assignment-proposals/${id}/reject`,
      { admin_note: adminNote },
    );
  },
};
...
export const pjBagianApi = {
  list: () => {
    return api.get<{ success: boolean; data: PjBagianUser[] }>(
      "/employees/pj-bagian",
    );
  },
...
export const employeeRegistrationApi = {
  ...
  show: (id: number) => {
    return api.get<{ success: boolean; data: EmployeeRegistration }>(
  ...
  reject: (id: number, adminNote: string) => {
    return api.put<{
      ...
      message: string;
      data: EmployeeRegistration;
    }>(`/employee-registrations/${id}/reject`, { admin_note: adminNote });
  },
...
export const resignationApi = {
  list: (params?: { personal?: boolean; status?: string; department_id?: number; search?: string }) => {
    const query = new URLSearchParams();
    if (params?.personal) query.append('personal', '1');
    if (params?.status) query.append('status', params.status);
    ...
    }>(`/resignation-requests?${query.toString()}`);
  },
  ...
  cancel: (id: number) => {
    return api.delete<{
      success: boolean;
      message: string;
    }>(`/resignation-requests/${id}/cancel`);
  },
  ...
  review: (id: number, status: 'approved' | 'rejected', adminNote?: string) => {
    return api.put<{
      ...
      message: string;
      data: ResignationRequest;
    }>(`/resignation-requests/${id}/review`, { status, admin_note: adminNote });
  },

  pjReview: (id: number, status: 'approved' | 'rejected', pjNote?: string) => {
    return api.put<{
      success: boolean;
      message: string;
      data: ResignationRequest;
...
export const disciplinarySanctionApi = {
  list: (params?: { personal?: boolean; type?: string; department_id?: number; search?: string }) => {
    const query = new URLSearchParams();
    if (params?.personal) query.append('personal', '1');
    ...
    return api.get<{
      success: boolean;
      data: DisciplinarySanction[];
      meta?: { current_page: number; last_page: number; per_page: number; total: number };
    }>(`/disciplinary-sanctions?${query.toString()}`);
  },
  ...
  create: (formData: FormData) => {
    return api.post<{
      ...
      message: string;
      data: DisciplinarySanction[];
    }>('/disciplinary-sanctions', formData);
  },
