    ...
    <>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4 pb-1">
        <div className="flex items-center gap-3">
          {onBack && (
            <button onClick={onBack} className="w-8 h-8 rounded-xl bg-white border border-gray-200 flex items-center justify-center hover:bg-gray-50 active:scale-95 transition-all shadow-xs cursor-pointer">
              <ChevronLeft size={16} className="text-gray-600" />
            </button>
          )}
          <div>
            <h1 className="text-[18px] md:text-[20px] font-bold text-gray-900">Pengajuan Cuti & Sakit</h1>
            <p className="text-[11.5px] md:text-[12.5px] text-gray-500 mt-0.5">Kelola pengajuan cuti, sakit, dan cuti khusus Anda</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4 pb-10">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 mb-4 hover:bg-amber-100 transition-colors group text-left cursor-pointer"
        >
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 mb-5">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-[#2563EB] hover:bg-[#1d4ed8] text-white rounded-2xl text-[13px] font-semibold transition-all shadow-sm shadow-blue-200 active:scale-[0.98] cursor-pointer"
          >
            <Plus size={16} /> Ajukan Cuti
          </button>
          <button
            onClick={() => { setShowForm(true); setLeaveType('sakit'); setFormError(''); }}
            className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-amber-700 hover:bg-amber-800 text-white rounded-2xl text-[13px] font-semibold transition-all shadow-sm shadow-amber-200 active:scale-[0.98] cursor-pointer"
          >
            <Plus size={16} /> Ajukan Sakit
          </button>
          <button
            onClick={() => { setShowForm(true); setLeaveType('cuti_khusus'); setFormError(''); }}
            className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-orange-500 hover:bg-orange-600 text-white rounded-2xl text-[13px] font-semibold transition-all shadow-sm shadow-orange-200 active:scale-[0.98] cursor-pointer"
          >
            <Plus size={16} /> Ajukan Cuti Khusus
          </button>
        </div>

        {/* ─── SUBMISSION FORM MODAL DIALOG ──────────────────────────────────── */}
        {showForm && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
            <div className="fixed inset-0 bg-black/50 backdrop-blur-xs animate-fade-in" onClick={() => setShowForm(false)} />
            <div className="relative bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden z-10 my-auto max-h-[92vh] flex flex-col font-sans border border-gray-100">
              {/* Form Header */}
