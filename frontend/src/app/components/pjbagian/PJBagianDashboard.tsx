                ...
                return (
                  <div key={sItem.id || sIdx} className="space-y-3 p-3.5 rounded-2xl bg-slate-50/60 border border-slate-150">
                    ...
                    <div className="grid grid-cols-3 gap-2.5">
                      {[
                        { label: 'Jam Masuk', value: sStart, sub: 'WIB', bg: '#F0FDF4' },
                        { label: 'Check-In Aktual', value: rItem?.check_in ? rItem.check_in.substring(0, 5) : '--:--', sub: rItem?.check_in ? (rItem.status === 'telat' ? 'Terlambat' : 'Tepat Waktu') : (isOffDuty ? (leaveType ?? 'Bebas Tugas') : 'Belum Absen'), bg: rItem?.check_in ? (rItem.status === 'telat' ? '#FFFBEB' : '#F0FDF4') : '#FFFFFF' },
                        { label: 'Jam Pulang', value: sEnd, sub: 'WIB', bg: '#F0FDF4' },
                      ].map((b, i) => (
                        <div key={i} className="rounded-xl p-2.5 text-center border border-slate-100" style={{ background: b.bg }}>
                          ...
                          <p className="text-[16px] font-bold font-mono text-gray-900">{b.value}</p>
