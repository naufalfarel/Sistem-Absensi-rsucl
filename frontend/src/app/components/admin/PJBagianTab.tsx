    ...
    <div className="space-y-4 max-w-5xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 bg-[#2563EB] hover:bg-[#1d4ed8] text-white rounded-xl text-[11px] font-bold transition-all shadow-xs active:scale-95"
        >
      ...
      {/* Info Warning */}
      <div className="p-3 bg-blue-50 border border-blue-150 text-[11px] rounded-xl text-blue-750 flex items-start gap-2">
        <ShieldAlert size={14} className="mt-0.5 flex-shrink-0 text-blue-600" />
        <span>
      ...
      <div className="bg-white rounded-xl border border-gray-150 overflow-hidden shadow-xs">
        <div className="p-4 border-b border-gray-100 flex items-center gap-2">
          ...
          <input
            type="text"
            placeholder="Cari PJ, NIP, atau departemen..."
            value={search}
                ...
                <tr key={pj.user_id} className="hover:bg-gray-50/50">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-lg bg-blue-50 flex items-center justify-center text-[#2563EB] font-bold">
                        {pj.name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p className="font-bold text-gray-800">{pj.name}</p>
                        <p className="text-[10px] text-gray-400">{pj.nip}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-gray-600 font-medium">{pj.position || 'Staf'}</td>
                  <td className="py-3 px-4">
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-50 text-[#2563EB] font-semibold">
                  ...
                  <td className="py-3 px-4 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-red-650 hover:bg-red-50 rounded-lg font-bold transition-all"
                    >
                      <Trash2 size={11} /> Cabut Wewenang
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl p-5 shadow-xl w-full max-w-sm z-10">
            <h3 className="text-[13px] font-bold text-gray-900 mb-1 flex items-center gap-1.5">
              <UserCheck size={16} className="text-[#2563EB]" /> Tugaskan PJ Bagian
            </h3>
            ...
            <form onSubmit={handleAssign} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-[11px] bg-gray-50 focus:outline-none focus:border-[#2563EB] transition-all font-semibold text-gray-700"
                >
                  <option value="">-- Pilih Karyawan --</option>
                  {employees.filter(e => e.role !== 'admin').map(emp => (
                    <option key={emp.id} value={emp.id}>{emp.name} ({emp.nip}) - {emp.department}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 py-2 bg-[#2563EB] hover:bg-[#1d4ed8] text-white rounded-lg text-[11px] font-bold transition-all shadow-xs active:scale-95 disabled:opacity-50"
                >
