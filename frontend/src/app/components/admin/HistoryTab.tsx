...
export function HistoryTab() {
  ...
  const isCardActive = (cardIndex: number) => {
    ...
    switch (cardIndex) {
      ...
      case 2:
        return (
          statusFilter.includes("tidak_lengkap") && statusFilter.length === 1
        );
      case 3:
        return (
          statusFilter.includes("alpha") &&
          statusFilter.includes("belum_hadir") &&
          statusFilter.length === 2
        );
      case 4:
        return (
          statusFilter.includes("cuti") &&
          statusFilter.includes("izin") &&
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-[12px] text-gray-400 mt-0.5">
            Rekap lengkap kehadiran seluruh karyawan
          </p>
        </div>
      </div>
      ...
      <MonthYearDeptFilter
        ...
        onMonthChange={(m) => {
          ...
          if (m > 0) {
            ...
            setDateTo(
              `${filterYear}-${mm}-${String(lastDay).padStart(2, "0")}`,
            );
          }
        }}
        onYearChange={(y) => {
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          {
            label: "Tidak Lengkap",
            value: (summary as any).tidak_lengkap || 0,
            color: "#4B5563",
          },
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <div className="relative">
          ...
          >
            ...
            <option value="izin,sakit,cuti,cuti_khusus">
              Cuti / Izin / Sakit
            </option>
          </select>
          <ChevronDown
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              ...
              {!loading &&
                records.map((r, i) => {
                  let sc = statusConfig[r.display_status || r.status] ||
                  ...
                  if (
                    r.display_status !== "tidak_lengkap" &&
                        ...
                        {isLeavePeriod ? (
                          ...
                          <>
                            <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-green-50 text-green-700">
                              {r.shift_name || "Reguler"}
                            </span>
                            {r.dinas_reason ? (
                              <span
                                className="block mt-1 text-[9px] font-bold text-purple-600 bg-purple-50 border border-purple-100 rounded-md px-1.5 py-0.5 w-max"
                                title={r.dinas_reason}
                              >
                                {r.dinas_reason}
                              </span>
                            ) : r.shift_type === "dinas_luar" ? (
                              <span className="block mt-1 text-[9px] font-bold text-purple-600 bg-purple-50 border border-purple-100 rounded-md px-1.5 py-0.5 w-max">
                                Dinas Luar
                              </span>
                            ) : null}
                            {r.is_holiday_work && (
                              <span
                                className="block mt-1 text-[9px] font-bold text-red-600 bg-red-50 border border-red-100 rounded-md px-1.5 py-0.5 w-max"
                                title={r.holiday || "Hari Libur"}
                              >
                                Kerja Libur
                              </span>
                            )}
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-white rounded-2xl p-6 shadow-2xl w-full max-w-sm">
            <div className="flex items-center gap-3 mb-5">
              <div
                onClick={() => {
                  if (selected.employee?.profile_picture) {
                    setPreviewPhoto({
                      url: selected.employee.profile_picture,
                ...
                style={{
                  background: (
                    statusConfig[selected.status] || { bg: "#F9FAFB" }
                  ).bg,
                }}
              >
                {selected.employee?.profile_picture ? (
                  <img
                    src={selected.employee.profile_picture}
                    alt={selected.employee.name}
                  ...
                  <span
                    ...
                    style={{
                      color: (
                        statusConfig[
                          selected.display_status || selected.status
                        ] || { color: "#6B7280" }
                      ).color,
                    }}
                  >
                    {(selected.employee?.name ?? "K")
              ...
              <div>
                ...
                <p className="text-[12px] text-gray-500">
                  {selected.employee?.department || "Umum"}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2.5 bg-gray-50 rounded-xl p-4 mb-5">
              {[
                { l: "Tanggal", v: formatDate(selected.date) },
                {
                  l: "Shift",
                  v:
                    (selected.shift_name || "Reguler") +
                    (selected.dinas_reason
                      ? ` (${selected.dinas_reason})`
                      : selected.shift_type === "dinas_luar"
                        ? " (Dinas Luar)"
                        : " (Normal)"),
                },
                {
                  ...
                  v:
                    (selected.check_in
                      ? selected.check_in.substring(0, 5)
                      : "--") +
                    (selected.checkin_distance_meters !== undefined &&
                    selected.checkin_distance_meters !== null
                      ? ` (${selected.checkin_distance_meters}m dari RSUCL)`
                      : ""),
                },
                {
                  l: "Jam Keluar",
                  v:
                    (selected.check_out
                      ? selected.check_out.substring(0, 5)
                      : "--") +
                    (selected.checkout_distance_meters !== undefined &&
                    selected.checkout_distance_meters !== null
                      ? ` (${selected.checkout_distance_meters}m dari RSUCL)`
                      : ""),
                },
                {
                  l: "Status",
                  v:
                    selected.display_status === "tidak_lengkap"
                      ? "Tidak Lengkap"
                      : (selected.status === "hadir" ||
                            selected.status === "telat") &&
                          selected.checkin_punctuality
                        ? selected.checkin_punctuality === "tepat_waktu"
                          ? "Tepat Waktu"
                          : selected.checkin_punctuality === "toleransi"
                            ? "Toleransi"
                            : "Terlambat"
                        : (
                ...
                ...(selected.checkin_location_note
                  ? [{ l: "Posisi", v: selected.checkin_location_note }]
                  : []),
                ...(selected.checkout_location_note
                  ? [{ l: "Lokasi Pulang", v: selected.checkout_location_note }]
                  : []),
              ].map(({ l, v }, i) => (
                <div key={i} className="flex justify-between">
                  <span className="text-[12px] text-gray-500">{l}</span>
                  <span className="text-[12px] font-medium text-gray-800 text-right max-w-[180px] break-words">
                    {v}
                  </span>
                </div>
              ))}
            ...
            {selected.is_lembur && (
              <div className="mb-5 p-3.5 bg-blue-50 border border-blue-200 rounded-xl space-y-2 text-left">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                      selected.status_approval_lembur === "disetujui"
                        ? "bg-green-100 text-green-700"
                        : selected.status_approval_lembur === "ditolak"
                ...
                <p className="text-[11.5px] text-gray-600">
                  <span className="font-semibold">Pekerjaan Lembur:</span>{" "}
                  {selected.keterangan_lembur || "-"}
                </p>
                  ...
                  <div className="flex gap-2 pt-2 border-t border-blue-200/50 mt-1">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          await attendanceApi.rejectOvertime(
                            selected.id,
                            reason,
                          );
            ...
            <button
              onClick={() => setSelected(null)}
              className="w-full py-2.5 bg-gray-100 rounded-xl text-[13px] font-medium text-gray-600 hover:bg-gray-200 transition-colors"
            >
              Tutup
            </button>
          </div>
